...
References resolve by *exact* match only, against the full ID, the slug, or the
full `<id>-<slug>` name, never by prefix or fuzzy match. A slug derives from the
mutable title and may be shared; a slug naming more than one issue does not
resolve, and the caller falls back to the ID. A stale on-disk name (its slug no longer matching
