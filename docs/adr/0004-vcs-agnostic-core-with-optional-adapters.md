...
Busy Beaver's core never requires, invokes, or assumes a version-control
...
be corrupted by rebases and squashes. Busy Beaver is Git-*friendly*, not
Git-*dependent*.
...
behavior is **opt-in** — by default Busy Beaver writes files and commits
