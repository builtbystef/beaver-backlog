...
per request, re-rendering from truth instead of reconciling a copy of it, with
every template and asset embedded via `go:embed`. A SPA and its build
...
rule; it bought a no-JS fallback nobody wanted here, given that this is a local
tool opened in a modern browser, at the price of every interactive feature
