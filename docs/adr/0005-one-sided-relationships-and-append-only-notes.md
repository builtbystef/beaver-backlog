...
(`parent: id`). Each relationship is stored on one side only, on the dependent
or the child, and the inverse (what an issue blocks, an issue's children) is
...
- A dependency on a `cancelled` issue never clears, so the dependent is
  *stuck*. `doctor` flags it for a human to cancel, re-scope, or drop the edge,
