# Busy Beaver is VCS-agnostic; the issue store is self-describing

Busy Beaver's core never requires, invokes, or assumes any version-control system.
Every operation works on files alone, so Busy Beaver functions correctly under Git,
...
frontmatter rather than being derived from VCS history. Busy Beaver never reconstructs
issue metadata from `git log` or any equivalent.

Deriving metadata from Git would re-couple Busy Beaver to Git, break before the first
...
0001/0003/0005: Busy Beaver is Git-*friendly*, not Git-*dependent*.

## Consequences

- Stored metadata can desync from reality if a file is hand-edited carelessly.
  Busy Beaver maintains `updated` on every write, and `doctor` can flag anomalies.
- Strong "history" (who changed what, when, across versions) is provided by
  whatever VCS is in use, when one is in use — it is not a core Busy Beaver guarantee.
