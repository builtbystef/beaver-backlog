...
- `cmd/beaver/` — the binary: wires the real process (args, stdio, filesystem,
  clock, VCS, editor) to the CLI engine and maps errors to stable exit codes.
- `internal/cli/` — one file per command, plus shared plumbing. The engine
  ...
  tests substitute args, stdio, clock, VCS, and editor wholesale.
...
- `internal/vcs/` — the VCS port and its Git adapter. The core is VCS-agnostic;
  Git is an optional adapter behind this seam (ADR 0004).
...
- **VCS port** (`internal/vcs`): core code depends on the port, not Git.
