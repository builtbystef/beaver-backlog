...
func (s *server) changed(w http.ResponseWriter, r *http.Request) {
	...
	// A store that cannot answer — deleted mid-session, a directory turned
	// unreadable — is an outage, not a change: the page keeps its last
...
// file names, sizes, and times around control characters — not legal in a
// header — so what travels is a digest, quoted as ETags are.
